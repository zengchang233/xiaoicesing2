...
class Xiaoice2(nn.Module):
    """ Xiaoice2 """

    def __init__(self, data_config, model_config):
        super(Xiaoice2, self).__init__()
        ...
        self.loss = FastSpeech2Loss(data_config)
    ...
    def forward(
        ...
        output = self.encoder(texts, note_pitchs, note_durations, src_masks)
        ...
        outputs = (output, postnet_output, p_predictions, e_predictions, uv_predictions, log_d_predictions, d_rounded, src_masks, mel_masks, src_lens, mel_lens)

        (total_loss, mel_loss, post_mel_loss, pitch_loss, energy_loss, uv_loss, duration_loss) = self.loss((mels, mel_lens, max_mel_len, p_targets, e_targets, uv_targets, d_targets), outputs)
...
if __name__ == "__main__":
    ...
    model = Xiaoice2(data_config, model_config['generator'])
    print(model)
